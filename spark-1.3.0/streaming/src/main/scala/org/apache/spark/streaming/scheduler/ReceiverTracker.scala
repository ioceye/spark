...
class ReceiverTracker(ssc: StreamingContext, skipReceiverLaunch: Boolean = false) extends Logging {
  /**
    * 从StreamingContext的graph中的对象。
    * 也就是,调用StreamingContext创建一个输入DSteam时,都会放入DStreamGraph的ReceiverInputStreams中。
    */
  ...
  def start() = synchronized {
    ...
    if (!receiverInputStreams.isEmpty) {
      actor = ssc.env.actorSystem.actorOf(Props(new ReceiverTrackerActor),
        "ReceiverTracker")
      // 调用ReceiverLauncher的start方法,启动receiver
  ...
  class ReceiverLauncher {
     ...
     */
    // ReceiverTracker是在Driver上执行的
    private def startReceivers() {
      // 调用所有输入DStream的getReceiver方法,拿到其对应的receivers集合
      ...
      /**
        * Receiver的核心逻辑,但是不是在Driver上执行的,只是定义了startReceiver的函数.
        * startReceiver函数将被分布到worker的executor上执行
        */
      val startReceiver = (iterator: Iterator[Receiver[_]]) => {
        ...
        // 将Receiver封装在ReceiverSupervisorImpl中,并调用start方法启动
      ...
      /**
        * 调用StreamingContext的SparkContext的runJob方法,将启动Receiver的函数,
        * 并且分布到worker的executor节点上执行
        */
