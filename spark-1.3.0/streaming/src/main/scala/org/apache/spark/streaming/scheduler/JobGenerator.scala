...
class JobGenerator(jobScheduler: JobScheduler) extends Logging {
  ...
  /**
    * 初始化任务开始时间。
    * 以及根据batchDuration, 每个batch时间将执行一系列的graph操作
    */
  ...
  private def generateJobs(time: Time) {
    ...
    Try {
      /**
        * 第一步,找到ReceiverTracker,调用其allocateBlocksToBatch方法,将当前时间段内
        * 的block分配给一个batch,并为其创建一个RDD
        */
      jobScheduler.receiverTracker.allocateBlocksToBatch(time) // allocate received blocks to batch
      /**
        * 然后,调用DStream的generateJobs方法,来根据我们定义的
        * DStream之间的依赖关系和算子,生成job
        */
      ...
      case Success(jobs) =>
        ...
        // 用JobScheduler提交Job,并且提交time时间段的数据
