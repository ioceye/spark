 ...
 */
/**
  * 创建以及完成StreamContext初始化之后,其实例化了DStreamGraph、JobScheduler等组件,
  * 就会调用StreamContext的socketTextStream等方法,创建输入DStream,然后针对输入的DStream
  * 执行一系列的transformation转换操作。
  * 最后执行output输出操作,触发batch的操作。
  */
class StreamingContext private[streaming] (
  ...
  /**
    * DStreamGraph保存了Spark Streaming Application中,定义的一系列DStream依赖关系,
    * 以及相互之间算子的作用
    */
  ...
  /**
    * JobGenerator每隔batch interval,创建一个job,然后通过JobScheduler调度job,
    * 底层还是spark核心计算引擎,DAGScheduler、TaskSheduler、Worker、Executor、Task。
    * 如果使用reduceByKey算子,还会有shuffle,而且底层的数据存储组件还是executor关联的BlockManager。
