...
private[streaming] class BlockGenerator(
  ...
  // 决定了,生成block的时间间隔,由配置spark.streaming.blockInterval完成,默认为200ms
  private val blockInterval = conf.getLong("spark.streaming.blockInterval", 200)
  // 每隔blockInterval,就会调用一次updateCurrentBuffer函数
  private val blockIntervalTimer =
    new RecurringTimer(clock, blockInterval, updateCurrentBuffer, "BlockGenerator")
  // spark.streaming.blockQueueSize控制blocksForPushing的容量,默认为10个数据
  private val blockQueueSize = conf.getInt("spark.streaming.blockQueueSize", 10)
  private val blocksForPushing = new ArrayBlockingQueue[Block](blockQueueSize)
  /**
    * 后台线程,调用keepPushingBlocks方法.
    * keepPushingBlocks方法定时从blocksForPushing队列中拿取数据。
    */
  ...
  def start() {
    /**
      * blockIntervalTimer： 负责将currentBuffer中的原始数据,打包为一个一个的block。
      * blockPushingThread： 负责将blocksForPushing中的block, 调用pushArrayBuffer方法
      */
  ...
  private def updateCurrentBuffer(time: Long): Unit = synchronized {
    try {
      // 打包currentBuffer为一个block,并且清空currentBuffer数据
