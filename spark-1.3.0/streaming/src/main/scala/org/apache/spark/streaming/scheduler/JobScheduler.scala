...
class JobScheduler(val ssc: StreamingContext) extends Logging {
  ...
  // StreamingContext的start方法,启动作业调度
  def start(): Unit = synchronized {
    ...
    listenerBus.start()
    // 创建ReceiverTracker组件,并且启动
    receiverTracker = new ReceiverTracker(ssc)

    /**
      * StreamingContext的相关组件已经创建完成,然后启动输入DStream
      * 相关联的Receiver,相关逻辑封装在了receiverTracker的start方法中。
      */
    receiverTracker.start()
    // 创建JobScheduler的时候创建,启动jobGenerator
    jobGenerator.start()
